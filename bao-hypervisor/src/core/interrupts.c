...
{
    if (interrupts_arch_irq_is_forwardable(int_id) && cpu()->vcpu != NULL &&
        vm_has_interrupt(cpu()->vcpu->vm, int_id)) {
...
bool interrupts_vm_assign(struct vm* vm, irqid_t id)
{
    ...
    if ((id < MAX_INTERRUPT_LINES) && interrupt_is_shared(id)) {
        ...
        ret = true;
    } else if ((id < MAX_INTERRUPT_LINES) && !interrupt_assigned(id) &&
        !interrupts_arch_conflict(global_interrupt_bitmap, id)) {
        ret = true;
        interrupts_arch_vm_assign(vm, id);
