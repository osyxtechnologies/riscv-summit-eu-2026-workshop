...
bool virtio_console_init(struct virtio_console *console, char *shmem_base, long mmio_base)
{
    ...
    while (virtq_has_free_slots(&console->vqs[VIRTIO_CONSOLE_RX_VQ_IDX]))
    {
        ...
        virtq_desc_init(desc, (uint64_t)(uintptr_t)io_buffer, VIRTIO_CONSOLE_RX_BUFFER_SIZE);
...
bool virtio_console_mmio_init(struct virtio_console *console)
{
    ...
    for (int vq_id = 0; vq_id < VIRTIO_CONSOLE_NUM_VQS; vq_id++)
    {
        ...
        console->mmio->QueueDescLow = (uint32_t)((uintptr_t)console->vqs[vq_id].desc & 0xFFFFFFFF);
        console->mmio->QueueDescHigh = (uint32_t)(((uint64_t)(uintptr_t)console->vqs[vq_id].desc >> 32) & 0xFFFFFFFF);
        console->mmio->QueueDriverLow = (uint32_t)((uintptr_t)console->vqs[vq_id].avail & 0xFFFFFFFF);
        console->mmio->QueueDriverHigh = (uint32_t)(((uint64_t)(uintptr_t)console->vqs[vq_id].avail >> 32) & 0xFFFFFFFF);
        console->mmio->QueueDeviceLow  = (uint32_t)((uintptr_t)console->vqs[vq_id].used & 0xFFFFFFFF);
        console->mmio->QueueDeviceHigh = (uint32_t)(((uint64_t)(uintptr_t)console->vqs[vq_id].used >> 32) & 0xFFFFFFFF);
...
void virtio_console_transmit(struct virtio_console *console, char *const data)
{
    ...
    /* Initialize the descriptor */
    virtq_desc_init(desc, (uint64_t)(uintptr_t)io_buffer, data_len);
    ...
    for (int vq_id = 0; vq_id < VIRTIO_CONSOLE_NUM_VQS; vq_id++)
    {
        ...
        while (virtq_used_has_buf(vq))
        {
            ...
            if (vq_id == VIRTIO_CONSOLE_RX_VQ_IDX) {
                ...
                char* msg = (char*)(uintptr_t)desc->addr;
            ...
            if(!virtio_memory_pool_free(&vq->pool, (char*)(uintptr_t)desc->addr, desc->len)) {
