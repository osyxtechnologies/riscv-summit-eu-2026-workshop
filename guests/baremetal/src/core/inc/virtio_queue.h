...
#define VIRTQ_DESC_F_NEXT       1ULL           /**< This marks a buffer as continuing via the next field */
#define VIRTQ_DESC_F_WRITE      2ULL           /**< This marks a buffer as device write-only (otherwise device read-only) */
#define VIRTQ_DESC_F_INDIRECT   4ULL           /**< This means the buffer contains a list of buffer descriptors */
#define VIRTQ_DESC_F_AVAIL      (1ULL << 7)    /**< This marks a buffer as available for the device to use */
#define VIRTQ_DESC_F_USED       (1ULL << 15)   /**< This marks a buffer as used by the device */
...
static inline void virtq_dump(struct virtq *vq)
{
    ...
    if (!vq->desc) {
        ...
        for (int i = 0; i < VIRTQ_SIZE; i++) {
            printf("desc[%d] addr = 0x%lx (data = %s), len = 0x%x, flags = 0x%x, next = 0x%x\n", i, vq->desc[i].addr, (char*)(uintptr_t)vq->desc[i].addr, vq->desc[i].len, vq->desc[i].flags, vq->desc[i].next);
        }
    }
    if (!vq->avail) {
